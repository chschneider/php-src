...
if (isset($opts['help'])) {
	fputs(STDERR, <<<EOF
		...
		Extract function documentation from doc-en (or a translation) reference
		and add them as @genstubs-expose-comment-blocks to the php stub files.
		You can call build/gen_stub.php --force to regenerate all stub files.

		Options:
		  -d, --doc-path=PATH  Path to doc-en (or translation) repository checkout
		  -h, --help           Show usage
...
foreach (glob("{$doc_path}/language-*.ent") as $entityfile) {
	...
	$entities += array_combine(array_map(fn($v) => "&$v;", $ent[1]), $ent[2]);
}
...
foreach (glob("$doc_path/reference/*/*/*.xml") as $file) {
	...
	do {
		$oldxml = $xml;
		$xml = str_replace(array_keys($entities), array_values($entities), $xml);	# Replace known custom entities
	} while ($oldxml != $xml);
	...
	if (array_all($params, fn($v) => !empty($v['type']))) {
		...
		foreach ($params as $param => $info) {
			if (empty($info['type']) || empty($info['description']))
...
#
foreach (array_merge(glob('*/*.stub.php'), glob('*/*/*.stub.php')) as $stubfile)
{
